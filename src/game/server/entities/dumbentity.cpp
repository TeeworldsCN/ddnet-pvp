...
void CDumbEntity::DoSnap(int SnapID, int SnappingClient)
{
	...
	if(Type <= TYPE_POWERUP_NINJA)
	{
		...
		CNetObj_Pickup *pP = static_cast<CNetObj_Pickup *>(Server()->SnapNewItem(NETOBJTYPE_PICKUP, 65535, Size));
		if(!pP)
			return;
	...
	else if(Type <= TYPE_PROJECTILE_GRENADE)
	{
		...
		else
		{
			...
			else
			{
				...
				if(fpclassify(ClientVel.x) == FP_ZERO)
					pProj->m_VelX = 0;
				else
					pProj->m_VelX = (int)(Direction.x * (Vel.x / ClientVel.x) * 100.0f);

				if(fpclassify(ClientVel.y) == FP_ZERO)
					pProj->m_VelY = 0;
				else
